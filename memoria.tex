...
El lenguaje de programación cada vez es más necesario que se empiece a trabajar en las aulas desde los inicios en la educación, igual que se enseña la lengua, las matemáticas, la historia, puesto que favorece la estructuración del pensamiento crítico y la lógica, esenciales en la resolución de problemas.
Difícilmente se consigue enseñar a un niño de unos 5 o 6 años a que escriba código de programación.
Pero si es posible que comience a familiarizarse con el mundo de la programación y su lenguaje general sin que se dé cuenta de una manera lúdica y al mismo tiempo formativa.
Como en las primeras edades sus capacidades lectoescritoras todavía están formándose, por medio de herramientas vinculadas a elementos visuales como juegos, puzzles, aventuras, podemos arrancar el desarrollo del pensamiento computacional recurriendo a su creatividad. Herramientas cómo las aplicaciones Kodable, Bit by bit, Codekarts y ScratchJr. 

De ésta última, ScratchJr (Sección~\ref{sec:scratchJr}), es donde parte este proyecto.
ScratchJr está basada en Scratch pero con su interfaz y lenguaje de programación rediseñados para niños más pequeños.
En esta universidad se desarrolló la herramienta Dr.Scratch que permite a los usuarios de Scratch analizar sus proyectos y medir sus capacidades de programación cuantificadas por medidores de habilidades de pensamiento computacional.
...
En ella profesores pueden analizar los proyectos que hayan hecho sus alumnos en ScratchJr.
...
\begin{itemize}
    ...
    Pequeñas descripciones de las tecnologías utilizadas en la realización de la herramienta Dr.ScratchJr.
    ...
    Pruebas con un proyecto de test para validar que la herramienta detecta todos los datos a analizar correctamente, una pequeña comprobación de la correcta carga de varios proyectos y otra de la exportación de los datos en csv.
    \item Capítulo~\ref{chap:conclusiones}: \textbf{Conclusiones}. 
    Varias secciones donde se trata la consecución de los objetivos, las asignaturas del Grado que me han servido para la realización del proyecto, aquello que he aprendido gracias a la elaboración de dicho proyecto y algunas ideas de la herramienta para futuras mejoras.
...
En Dr.Scratch los usuarios pueden evaluar sus propios proyectos creados con Scratch, pero como los destinatarios de ScratchJr son niños, el objetivo real es crear una aplicación web con la función de ser una herramienta para que profesores o padres puedan analizar los proyectos de ScratchJr de los niños.
Y de esta manera poder facilitar y agilizar el trabajo de evaluación y guiar a los niños a un mayor aprendizaje.??? (detectando los errores más frecuentes que suelen darse en la programación).
% DUDA: mirar word
...
\begin{itemize} % DUDA: mirar word ?
    \item Investigar sobre los lenguajes de programación destinados a niños que se inician en ella. ???
    ...
    \item Crear los algoritmos para tratar y adaptar los datos a la aplicación web.
    ...
    \item Desplegar la aplicación web a un servidor web público.
...
ScratchJr es un lenguaje de programación de iniciación que permite a los niños pequeños crear sus propias historias y juegos interactivos.
...
Python es un lenguaje de programación de alto nivel, interpretado, dinámico, fuertemente tipado y multiplataforma.
...
Django es un framework de desarrollo web de alto nivel escrito en Python (capítulo~\ref{sec:python}).
...
Es una aplicación extensible que proporciona la funcionalidad de registro de usuarios para sitios web con tecnología Django (capítulo~\ref{sec:django}).
...
HTML5 es la última versión de HTML, la quinta, lanzada en 2014. Es un lenguaje de marcado que se utiliza para estructurar y desplegar páginas web.
...
\vspace{5mm}
Incorpora APIs (Application Programming Interfaces o Interfaces de programación de aplicaciones) que permiten incluir, a través de JavaScript (capítulo~\ref{sec:javascript}) componentes complejos.
Algunas destacadas son:

\begin{itemize}
    \item \textbf{Web Storage:} permite almacenar información en la parte del cliente, es decir, en el navegador web.
    Se divide en dos vertientes, ``SessionStorage'', para guardar información que caducará al final de la sesión y ``LocalStorage'', que permite almacenar datos que perduren entre distintas visitas del mismo usuario al sitio web.
    \item \textbf{Canvas:} permite añadir elementos gráficos.
    \item \textbf{Drag\&Drop:} nos permite tomar un elemento y arrastrarlo a una localización diferente.
    En HTML5 cualquier elemento puede ser \textit{draggable} (arrastrable).
    \item \textbf{Web Workers:} permiten ejecutar scripts en segundo plano (background), sin ser interrumpidos por eventos producidos por interacciones del usuario con la interfaz.
    \item \textbf{Geolocation:} permite averiguar la posición del usuario, en términos de coordenadas geográficas de latitud y longitud.
\end{itemize}
...
CSS es un lenguaje que sirve para crear páginas web o interfaces visualmente atractivas.
...
Esta separación puede darse dentro del propio archivo HTML o separándolo en una hoja de estilo .css. 
...
JavaScript (JS) es un lenguaje de programación interpretado, dinámico y débilmente tipado.
...
Bootstrap es un framework front-end utilizado para desarrollar sitios y aplicaciones web.
...
Mediante reglas media queries adapta la representación del contenido a características del dispositivo haciendo uso del sistema de rejillas (Grid System) de forma dinámica.
...
Pandas es una biblioteca construida sobre el lenguaje de programación Python de manipulación y análisis de datos.
...
En este capítulo se describe la arquitectura y como se ha diseñado la aplicación Dr.ScratchJr.
...
La arquitectura que sigue Dr.ScratchJr es el modelo cliente-servidor.
...
El navegador manda una petición HTTP a través de una URL que es interpretada por el mapa de URLS (URLmap o URLconf) para encontrar la vista apropiada.
...
Es a partir de este archivo SJR donde partimos.
...
\begin{figure}[H]
  ...
  \caption{Json en base de datos.}\label{fig:bd_json}
\end{figure}
...
\begin{itemize}
    ...
    El algoritmo, para cada uno de los tipos, devuelve los bloques que se han empleado al menos una vez en todo el proyecto.
    En Dr.ScratchJr se ha tomado como la misma clase de bloque aquellos que hacen la misma función pero de manera opuesta.
    ...
    Se puede observar en la figuras ~\ref{fig:movimiento} y ~\ref{fig:apariencia}.
    ...
    \begin{figure}[H]
     ...
     \caption{Tipos de bloques} \label{fig:bloques}
    \end{figure}
    \newpage
    ...
    \item En el criterio \emph{Creatividad} se busca si el usuario ha generado nuevo contenido respecto al contenido que proporciona la aplicación ScratchJr.
...
\begin{figure}[H]
  ...
  \caption{Criterios del análisis en base de datos.}\label{fig:bd_criterios}
\end{figure}
...
\begin{figure}[H]
  ...
  \caption{Análisis en base de datos.}\label{fig:bd_analisis}
\end{figure}

\vspace{5mm}
El proceso para el análisis de múltiples proyectos es el mismo salvo que en lugar de renderizarse ?? primero la plantilla del análisis de un proyecto, se renderiza ?? una plantilla con un listado de los proyectos subidos junto al estudiante asociado a ese proyecto (figura~\ref{fig:listado_zip}).
A partir de ese listado se puede ver el análisis de cada proyecto individualmente.

\begin{figure}[H]
  \centering
  \includegraphics[width=16cm, keepaspectratio]{img/listado_zip.png}
...
Al exportar se genera un zip, los datos se almacenan en tres ficheros csv (badhabits-1, otherdata-1, variability-1) y en una hoja de cálculo de Microsoft Excel llamada data.xlsx con los mismos datos de los tres csv, pero agrupados en pestañas en un solo documento.
...
En el caso de que estés registrado en Dr.ScratchJr aparecerá también un recuadro donde podrás asociar dicho proyecto con su autor si este ya existe o crear uno nuevo, y poder almacenar su análisis para posteriores visualizaciones.
...
\subsection{Direcciones urls}
\label{subsec:Tabla de urls con sus vistas}


La tabla ~\ref{tabla:path-url} hace un breve resumen de la funcionalidad de las vistas que están asociadas a las urls de la aplicacion web Dr.ScratchJr.
...
\begin{itemize}
    ...
    Cabe destacar que Dr.ScratchJr toma como el mismo bloque aquellos que hacen la misma función, pero de manera opuesta, como se puede observar en las figura ~\ref{fig:bloques}.
    ...
    \end{figure}
    Al pinchar sobre las cantidades de páginas o personajes editados, puedes ver la imagen correspondiente a cada página o personaje editado (figura ~\ref{fig:personajes-editados-detalles}).
    Además pinchando en la imagen puedes verla en tamaño completo en una pestaña nueva.
    \begin{figure}[H]
        \centering
        \includegraphics[width=15cm, keepaspectratio]{img/creatividad-detalles-personajes.PNG}
    ...
    \begin{figure}[H]
        \centering
        \includegraphics[width=9cm, keepaspectratio]{img/otros_datos.PNG}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=13cm, keepaspectratio]{img/otros_datos_Test1.png}
